...
int main( int argc, char** argv )
{
	...
	{
		if(i%20 == 0) std::cout << "Frames: " << i << " / 4541" << std::endl;

		cv::Mat image = cv::imread(files[i], CV_LOAD_IMAGE_GRAYSCALE);
		cv::Mat imageDist;
		cv::resize(image, imageDist, cv::Size(1280, 384));
	...
	profiler::dumpBlocksToFile("/root/kitti/LSD_SLAM_profile.prof");
