...
void SlamSystem::constraintSearchThreadLoop()
{
	...
	while(keepRunning)
	{
		...
		else
		{
			...
			EASY_BLOCK("FindConstraints", profiler::colors::Pink);
...
{
	EASY_BLOCK("ChangeKeyframe", profiler::colors::Yellow);
...
{
	EASY_BLOCK("UpdateKeyFrame", profiler::colors::Orange);
...
void SlamSystem::trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
	...
	struct timeval tv_start, tv_end;
	gettimeofday(&tv_start, NULL);
	EASY_BLOCK("Tracking", profiler::colors::Red);
	...
	EASY_BLOCK("MapEstimation", profiler::colors::Green);
...
bool SlamSystem::optimizationIteration(int itsPerTry, float minChange)
{
	struct timeval tv_start, tv_end;
	gettimeofday(&tv_start, NULL);
	EASY_BLOCK("Optimization", profiler::colors::Blue);
